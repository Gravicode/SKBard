...
namespace Connectors.AI.PaLM.Helper
{
	public class PaLMClient
	{
        ...
        public virtual async Task<string> GetMessageAsync(PaLMChatHistory history,AIRequestSettings settings, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                history.ForEach(x => { if (x.Role != AuthorRole.System) { hist.Add(new MessageRequest() { content = x.Content }); } });
